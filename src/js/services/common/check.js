linker.service('check', function() {
	...
	var credit_check = function(creditnum) {
		if (parseInt(creditnum) >= 0 && parseInt(creditnum) <= 999999) {
			return true;
		} else {
			return false;
		}
	}
	var compare_size = function(num, com_num) {
		if (parseInt(num)>=1&&parseInt(num)<=parseInt(com_num)){
			return true;
		}else{
			return false;
		}
