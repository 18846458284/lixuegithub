linker.controller('MaeOrderFlowController', ['$scope', '$window', '$state', '$location', 'maeOrdersService', 'responseService', 'webService',
...
//				webService.get_data("/getAgents").then(function(data) {
...
//					});
		...
		$scope.filterOrder = function() {
			if (checkDate()&&checkPhone()) {
				...
				$scope.timeStamp=new Date().getTime();
					$location.path("/admin/maeorderflow/" + $scope.mo + "/"+$scope.phoneNo +"/" + document.getElementById("fromDate").value + "/" + document.getElementById("toDate").value + "/" + $scope.spec +"/"+$scope.timeStamp);
				}
				else {
					$scope.spec = "";
						layer.alert('请输入正确规格，单位M或G(大小写都可)', {
							icon: 0
						});
					}
				}
