...
linker.controller('flowChannelController', function($scope, $window, $state, $location, check, responseService, webService) {
	$scope.page = 1;
	$scope.pageSize = 10;
	$scope.totalPage = 1;
	...
	$scope.searchchannel = function() {
		...
		if (test() && check() && checkName()) {
			$scope.timeStamp = new Date().getTime();
			$location.path("/admin/flowChannel/" + $scope.channelId + "/" + $scope.state + "/" + $scope.channelName + "/" + $scope.mo + "/" + $scope.province + "/" + $scope.spec + "/" + $scope.bizType + "/" + $scope.needSms + "/" + $scope.timeStamp);
		}
	};
	initial();
});