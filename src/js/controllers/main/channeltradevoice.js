linker.controller('ChannelTradeVoiceController', ['$scope', '$window', '$state', '$location', 'channelTradeService', 'responseService', 'webService',
	function($scope, $window, $state, $location, channelTradeService, responseService, webService) {
		...
		$scope.filterChannelTrade = function() {
			if (checkDate()&&checkName()) {
				$scope.timeStamp = new Date().getTime();
				$location.path("/admin/channeltradevoice/" + $scope.mo + "/" + $scope.province + "/" + document.getElementById("fromDate").value + "/" + document.getElementById("toDate").value + "/" + $scope.resourceId + "/" + $scope.bizType + "/" + $scope.timeStamp);
			}
		};
