linker.controller('ChannelQualityVoiceController', ['$scope', '$window', '$state', '$location', 'channelQualityService', 'responseService', 'webService',
	function($scope, $window, $state, $location, channelQualityService, responseService, webService) {
		...
		$scope.filterChannelQuality = function() {
			if (checkDate()&&checkName()) {
				$scope.timeStamp = new Date().getTime();
				$location.path("/admin/channelqualityvoice/" + $scope.mo + "/" + $scope.province + "/" + document.getElementById("fromDate").value + "/" + document.getElementById("toDate").value + "/" + $scope.resourceId + "/" + $scope.bizType + "/" + $scope.timeStamp);
			}
		};
