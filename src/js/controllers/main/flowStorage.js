linker.controller('flowStorageController', ['$scope', '$window', '$state', '$location', 'storageService', 'responseService', 'webService',
	function($scope, $window, $state, $location, storageService, responseService, webService) {
		...
		$scope.filterStorage = function() {
			...
			if (test() && check() && checkName()) {
				$location.path("/admin/flowStorage/" + $scope.storage_appId + "/" + $scope.appendState + "/" + $scope.goodsName + "/" + $scope.discount + "/" + $scope.mo + "/" + $scope.spec + "/" + $scope.state + "/" + $scope.sourceProvince + "/" + "flow" + "/" + $scope.timeStamp);
			}
		};
